...
function Sidebar({ activeTab, handleTabChange, handleLogout, name, image }) {
  return (
    <div className="flex w-1/4 flex-col py-8 px-6 glass-card shadow-lg ml-4">
      <div className="mb-10 text-center">
        ...
        />
        <h1 className="text-xl font-bold text-white">{name || "User Name"}</h1>
        <p className="text-sm text-slate-400 mt-1">FoodOS Member</p>
      </div>
      ...
      <div className="flex flex-col gap-3">
        {[
          ...
          <button
            key={tab.id}
            className={`rounded-lg px-4 py-3 text-left font-medium transition-colors ${activeTab === tab.id
      ...
      <button
        ...
        className="mt-auto flex items-center justify-center gap-2 rounded-lg bg-red-800 px-4 py-3 font-medium text-white transition-colors hover:bg-red-600"
      >
...
function ProfileForm({ name, setName, mobileNumber, setMobileNumber, address, setAddress, isEditing, setIsEditing }) {
  ...
  return (
    <form
      ...
      className="mx-auto h-full glass-card p-8 shadow-lg border-white/10"
    >
      <h2 className="mb-6 text-2xl font-bold text-white border-b border-white/5 pb-4">Personal Information</h2>
      <div className="space-y-6 max-w-2xl">
        {[
          ...
          <div key={id} className="">
            <label className="mb-2 block text-sm font-semibold text-slate-400" htmlFor={id}>
              {label}
            </label>
            {id === "address" ? (
              <textarea
                className={`w-full appearance-none rounded-lg border ${isEditing ? 'border-amber-500/50 focus:border-amber-500 focus:ring-1 focus:ring-amber-500' : 'border-white/10 bg-white/5'} px-4 py-3 text-white transition-colors focus:outline-none min-h-[100px] placeholder:text-slate-500`}
                id={id}
                name={id}
                value={value}
              ...
              <input
                className={`w-full appearance-none rounded-lg border ${isEditing ? 'border-amber-500/50 focus:border-amber-500 focus:ring-1 focus:ring-amber-500' : 'border-white/10 bg-white/5'} px-4 py-3 text-white transition-colors focus:outline-none placeholder:text-slate-500`}
                id={id}
                type="text"
                name={id}
                value={value}
      ...
      <div className="mt-8 pt-6 border-t border-gray-100 flex gap-4">
        {!isEditing ? (
          <button
            className="rounded-lg bg-white/10 px-6 py-3 font-medium text-white transition-colors hover:bg-white/20 border border-white/10"
            onClick={handleClick}
          >
            Edit Profile
          </button>
          ...
          <>
            ...
            <button
              className="rounded-lg bg-white/10 px-6 py-3 font-medium text-slate-300 transition-colors hover:bg-white/20 border border-white/10"
              onClick={handleClick}
            >
              Cancel
            </button>
...
function AddressManager({ user }) {
  ...
  return (
    <div className="h-full glass-card p-8 shadow-lg overflow-y-auto border-white/10">
      ...
      {isAdding && (
        <div className="bg-amber-500/5 p-6 rounded-xl border border-amber-500/10 mb-6 space-y-4">
          <div>
            <label className="block text-[10px] font-black text-amber-500 uppercase tracking-widest mb-1.5">Label (e.g. Home, Work)</label>
            <input
              type="text" value={newAddr.label} onChange={e => setNewAddr({ ...newAddr, label: e.target.value })}
              className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white text-sm outline-none focus:ring-2 focus:ring-amber-500 placeholder:text-slate-500"
            />
          </div>
          <div>
            <label className="block text-[10px] font-black text-amber-500 uppercase tracking-widest mb-1.5">Full Address</label>
            <textarea
              value={newAddr.text} onChange={e => setNewAddr({ ...newAddr, text: e.target.value })}
              className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white text-sm outline-none focus:ring-2 focus:ring-amber-500 min-h-[80px] placeholder:text-slate-500"
            />
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={() => setIsAdding(false)} className="px-6 text-slate-500 font-bold text-xs uppercase tracking-widest hover:text-white">Cancel</button>
          </div>
        </div>
      )}

      <div className="space-y-4">
        {loading ? [1, 2].map(i => <div key={i} className="h-24 bg-white/5 rounded-xl animate-pulse"></div>) :
          addresses.length === 0 ? <p className="text-slate-500 italic text-center py-10">No saved addresses yet.</p> :
            addresses.map(addr => (
              <div key={addr.address_id} className="p-5 border border-white/10 rounded-xl flex justify-between items-start group hover:border-amber-500/30 transition-all bg-white/5">
                <div className="flex gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-white flex items-center gap-2">
                      {addr.address_label}
                      {addr.is_default && <span className="text-[8px] bg-amber-500/20 text-amber-500 px-1.5 py-0.5 rounded-full uppercase border border-amber-500/20">Default</span>}
                    </h4>
                    <p className="text-xs text-slate-400 font-medium leading-relaxed max-w-md mt-1">{addr.address_text}</p>
                  </div>
                </div>
                <button onClick={() => deleteAddress(addr.address_id)} className="text-slate-500 hover:text-red-500 transition-colors">
...
function FavoritesList({ user }) {
  ...
  return (
    <div className="h-full glass-card p-8 shadow-lg overflow-y-auto border-white/10">
      <h2 className="text-2xl font-bold text-white mb-6 border-b border-white/5 pb-4">My Favorites</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {loading ? [1, 2, 3, 4].map(i => <div key={i} className="h-32 bg-white/5 rounded-xl animate-pulse"></div>) :
          favs.length === 0 ? <p className="text-slate-500 italic text-center py-10 col-span-full">No favorites yet.</p> :
            favs.map(fav => (
              <div key={fav.favorite_id} className="group relative bg-white/5 rounded-xl overflow-hidden border border-white/10 hover:shadow-md transition-all">
                <img src={fav.restaurants?.image_url} className="w-full h-32 object-cover opacity-50 group-hover:opacity-100" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-4">
                  <h4 className="text-white font-bold text-lg leading-tight">{fav.restaurants?.restaurant_name}</h4>
                  <p className="text-amber-500 text-[10px] font-bold uppercase tracking-widest">{fav.restaurants?.cuisine_type}</p>
                </div>
                <button
            ...
            {activeTab === "currentOrders" && (
              activeOrder ? (
                activeOrderStatus === "Dispatched" ? (
                  ...
                  <div className="glass-card p-12 text-center h-full flex flex-col items-center justify-center shadow-lg border-white/10">
                    <div className="w-24 h-24 bg-amber-500/10 rounded-full flex items-center justify-center text-amber-500 mb-6 relative">
                    ...
                    <p className="text-slate-400 max-w-sm">The restaurant is currently preparing your delicious meal. We'll start live tracking as soon as it's dispatched!</p>
                    <div className="mt-8 px-6 py-2 bg-amber-500/20 text-amber-500 rounded-full text-xs font-black uppercase tracking-widest border border-amber-500/20">
                ...
                <div className="glass-card p-12 text-center h-full flex flex-col items-center justify-center shadow-lg border-white/10">
                  <div className="w-20 h-20 bg-amber-500/10 rounded-full flex items-center justify-center text-amber-500 text-3xl mb-4">
                    <i className="ri-truck-line"></i>
                  </div>
                  <h2 className="text-2xl font-bold text-white mb-2">No Active Orders</h2>
                  <p className="text-slate-400">Your live delivery tracker will appear here after you place an order.</p>
                </div>
              )
            )}
