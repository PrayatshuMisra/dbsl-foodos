...
import { Search, Filter, SlidersHorizontal, Star, IndianRupee, Clock } from "lucide-react";
...
export default function Restos() {
  ...
  const filteredRestaurants = useMemo(() => {
    return restaurants.filter(restaurant => {
      ...
      // Cuisine filter
      const matchesCuisine = activeCuisine === "All" || restaurant.cuisine_type === activeCuisine;

      // Rating filter
      const matchesRating = restaurant.rating >= minRating;

      // Price filter (Rough estimation for demo)
      const matchesPrice = priceRange === "All" || 
        (priceRange === "Low" && restaurant.priceForTwo <= 500) ||
        (priceRange === "Mid" && restaurant.priceForTwo > 500 && restaurant.priceForTwo <= 1000) ||
        (priceRange === "High" && restaurant.priceForTwo > 1000);

      return matchesSearch && matchesCategory && matchesCuisine && matchesRating && matchesPrice;
    }).sort((a, b) => {
  ...
  }, [restaurants, searchQuery, categoryQuery, activeCuisine, minRating, priceRange, sortBy]);
    ...
    <div className="min-h-screen bg-white pt-24">
      ...
      <div className="container mx-auto px-4 lg:px-8 py-8 min-h-[70vh]">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-4xl font-black text-gray-900 tracking-tight">{pageTitle}</h1>
        ...
        {/* Updated Filter Bar */}
        <div className="sticky top-24 z-30 bg-white/80 backdrop-blur-md py-4 mb-8 border-b border-gray-100 flex flex-wrap gap-3 items-center">
          ...
          <select 
            ...
            className="px-4 py-1.5 bg-white border border-gray-200 rounded-full text-sm font-semibold hover:border-amber-500 focus:outline-none transition-colors cursor-pointer"
          >
            ...
            {[0, 3, 4, 4.5].map(r => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-bold transition-all ${
                  minRating === r ? "bg-white text-amber-600 shadow-sm" : "text-gray-500 hover:text-gray-800"
                }`}
              >
                {r === 0 ? "Any" : `${r}+`} <Star size={10} className="inline ml-0.5 fill-current" />
              </button>
            ))}
          </div>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <select 
              ...
              className="px-4 py-1.5 bg-gray-900 text-white rounded-full text-xs font-bold focus:outline-none cursor-pointer"
            >
