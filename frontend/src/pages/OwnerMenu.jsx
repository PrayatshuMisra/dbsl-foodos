      ...
      <div className="min-h-screen bg-transparent flex flex-col">
        ...
        <div className="flex-grow flex items-center justify-center p-6 mt-20">
          <div className="glass-card shadow-xl p-12 text-center max-w-md">
            ...
            <p className="text-slate-400">You don't have permission to manage menus. This area is reserved for restaurant partners.</p>
            <button
    ...
    <div className="min-h-screen bg-transparent flex flex-col">
      ...
      <div className="mx-auto flex gap-6 pt-24 pb-12 w-[90%] max-w-7xl flex-grow font-outfit">
        ...
        <div className="w-[75%] space-y-8">
              ...
              <div className="flex gap-4 mb-2">
                {['menu', 'promotions'].map(t => (
                  <button
                    ...
                    className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border transition-all ${activeTab === t ? 'bg-amber-500 border-amber-500 text-white shadow-md shadow-amber-200' : 'bg-white/5 border-white/10 text-slate-400 hover:border-amber-500 hover:text-amber-500'
                      }`}
                  >
                    {t}
                  </button>
          ...
          {/* Control Bar */}
          <div className="glass-card p-4 shadow-md flex flex-wrap gap-4 items-center">
            <div className="flex items-center gap-3 bg-white/5 px-4 py-2.5 rounded-lg border border-white/10 flex-grow max-w-md focus-within:ring-2 focus-within:ring-amber-500 transition-all">
              <FiSearch size={18} className="text-slate-500" />
              <input type="text" placeholder="Search your menu..." className="bg-transparent border-none outline-none text-sm font-medium w-full text-white placeholder:text-slate-500" />
            </div>
            <button className="px-4 py-2.5 bg-white/5 text-slate-300 rounded-lg font-bold text-xs flex items-center gap-2 border border-white/10 hover:bg-white/10 transition-all">
          ...
          {activeTab === 'menu' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {loading ? (
                ...
                <motion.div
                  layout
                  key={item.item_id}
                  className={`group glass-card overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 ${!item.availability_status && 'opacity-70'}`}
                >
                  ...
                  <div className="p-5 space-y-3">
                    <div className="flex justify-between items-start">
                      <h3 className="font-bold text-white truncate pr-2">{item.item_name}</h3>
                    ...
                    <button
                      ...
                      className={`w-full py-2.5 rounded-lg font-bold text-[10px] uppercase tracking-widest border transition-all ${item.availability_status
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {coupons.map(coupon => (
                <div key={coupon.coupon_id} className="glass-card p-6 border-dashed shadow-md relative overflow-hidden group">
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <span className="text-[10px] font-black text-amber-500 bg-amber-500/10 px-2 py-0.5 rounded-full uppercase tracking-widest">
                        {coupon.discount_type === 'PERCENTAGE' ? `${coupon.discount_value}% OFF` : `₹${coupon.discount_value} OFF`}
                      </span>
                      <h3 className="text-2xl font-black text-white mt-2 tracking-tight">{coupon.coupon_code}</h3>
                    </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden p-8 border border-white/10"
            >
